...
データの整理・操作は `tidyverse` というパッケージに含まれる関数を使うことが多い．
<!--
...
# Example using The Beatles data frame 
...
`filter` を適用した後のデータセットを新しいオブジェクトに割り当てるには `<-` を用いる．
...
以下のようにすれば元のオブジェクトを上書きすることも可能（以下同様）．

```{r beatles_filter_assign_2, eval = FALSE}
# beatles <- beatles %>% filter(born >= 1941)
# beatles %<>% filter(born >= 1941)  # same as above 
```
...
beatles %>% filter(born >= 1941 & height < 180)  # same as above 
```
...
### If some key exists in only one of the data frames / 片方のデータフレームにしかないキーがある場合 

一方のデータフレームにしかないキー（ID）がある場合，主たるデータフレーム（左側にある，マージされる側）の行はそのまま残る．
...
Case 1. 担当楽器のデータセット（右側）に対応する行がない場合．
...
Case 2. 逆に，主なデータフレーム（左側）の方に一部の行が存在しない場合．
...
beatles %>% summarise(mean_height = mean(height), 
                      ...
                      sample_size = n())
```
<!-- sd は観測数1の場合 NA になる；欠損値を除いた観測数を示す場合は n() ではなく sum(!is.na(var)) -->
...
beatles %>% 
  ...
  summarise(mean_height = mean(height), 
            std_dev_height = sd(height),  # n=1 なら sd=NA
            sample_size = n())
```
...
ggplot(data = beatles, mapping = aes(x = born, y = height)) + 
  geom_point() + 
  xlab("Year of birth")  # または labs(x = "...", y = "...") 
```
...
```{r titanic_filter}
titanic %>% filter(age > 75)  # NA は FALSE 扱い
```
...
titanic %>% 
  ...
  ggplot(mapping = aes(x = embarked, y = passengers)) + 
  geom_bar(stat = "identity") +  # または geom_col() 
  theme_classic()
```
...
titanic %>% 
  ...
  summarise(passengers = n()) %>% 
  ggplot(mapping = aes(x = embarked, y = passengers, fill = sex)) + 
...
titanic %>% 
  ...
  group_by(embarked) %>% 
  summarise(passengers = n()) %>% 
  ggplot(mapping = aes(x = "x", y = passengers, fill = embarked)) + 
...
* `pclass` は数値として記録されている質的変数（順序尺度）なので `factor` 関数で因子型の変数に変換して使う．
...
Base R （Rインストール時にbuilt-inされている標準パッケージのみを使用し，追加のパッケージを手動でインストールしない状態）でもパイプ演算子が使える．

- これは比較的新しい機能．`tidyverse` スタイルの `%>%` を用いたデータハンドリングが普及したことを受けて2021年頃に実装された．
- Base R のパイプは `|>` （縦棒と大なり記号を組み合わせたもの）と書く．
- パイプ `|>` を `base::subset` や `base::transform` などの base R の関数と組み合わせて使えば，`tidyverse` パッケージなしで `tidyverse` に似た方法で処理を記述することができる．
...
```{r baser_pipe_filter}
titanic %>% dplyr::filter(age >= 75)
titanic |> base::subset(age >= 75)
```

## `transform` corresponding to `mutate` 

```{r baser_pipe_mutate}
titanic %>% dplyr::mutate(family = sibsp + parch) %>% head(2)
titanic |> base::transform(family = sibsp + parch) |> head(2)
```

## `aggregate` corresponding to `group_by` + `summarise` 

```{r baser_pipe_groupby}
titanic %>% dplyr::group_by(pclass) %>% dplyr::summarise(survived_rate = mean(survived))
titanic |> stats::aggregate(survived ~ pclass, data = _, FUN = mean)
```

複数の変数を基準として集計することも可能（出力は省略）．

```{r baser_pipe_groupby_multi, eval = FALSE}
titanic %>% dplyr::group_by(pclass, sex) %>% dplyr::summarise(survived_rate = mean(survived))
titanic |> stats::aggregate(survived ~ pclass + sex, data = _, FUN = mean)
```
