...
ck %>% 
  group_by(state) %>% 
  summarize(BurgerKing = mean(chain == 1),
            KFC = mean(chain == 2),
...
ck %>% 
  group_by(state) %>% 
  summarize(sample_size = sum(!is.na(fte)),
...
宿題：上で計算していない他の変数やWave2についても同様に計算する．
...
(20.1-21.1) - (23.7-21.8)  # diff in diff
