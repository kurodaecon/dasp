...
### Three observations: $x_1 = 1, x_2= 0, x_3 = 1$
...
先ほどと同様に計算してみよう．
各行一つ目の値が $p$，二つ目が $p^2(1-p)$ を示す．

```{r cointoss_three_obs}
...
$$ f (\mu, \sigma^2 \mid y_i) = \frac{1}{\sqrt{2 \pi \sigma^2}} \exp \left[- \frac{(y_i - \mu)^2}{2 \sigma^2} \right] $$
$$ \log f (\mu, \sigma^2 \mid y_i) = - \frac{1}{2} \log 2 \pi - \frac{1}{2} \log \sigma^2 - \frac{(y_i - \mu)^2}{2 \sigma^2} $$
