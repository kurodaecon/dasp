...
for (p in p_vals) {
  barplot(c(1 - p, p),
    names.arg = c("X = 0", "X = 1"), ylim = c(0, 1), main = paste0("p = ", p), ylab = "")
}
par(mfrow = c(1, 1))
```
...
$$ \Pr(X_1 = 1, X_2 = 1, X_3 = 0) = 0.5 \times 0.5 \times 0.5 = 0.125 $$
...
$$ L(p \mid x) = \Pr(\mbox{Heads})^2 \times \Pr(\mbox{Tails})^1 = p^2 (1-p)^1, $$
$$ \log L(p \mid x) = 2 \log (p) + \log (1-p) $$

一階の条件（$L(\hat{p} \mid x)' = 0$ または $\log L(\hat{p} \mid x)' = 0$）より $\hat{p} = 2/3$ が得られる．

$$ \frac{d L(p \mid x)}{d p} = 2p (1-p) + p^2 (-1) = p(2-3p) = 0, $$
$$ \frac{d \log L(p \mid x)}{d p} = \frac{2}{p} + \frac{-1}{1-p} = \frac{2(1-p) - p}{p(1-p)} = \frac{2-3p}{p(1-p)} = 0, $$
