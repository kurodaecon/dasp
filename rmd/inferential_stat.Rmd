...
たとえば，一様分布から乱数をドローするには，`runif`関数を使う．この関数名は r (random number generation/generator) + unif (uniform distribution) から来ている．
<!--
...
連続確率変数について所与の区間の値を取る確率は密度関数 $f$ を用いて次で与えられる（ただし $\mbox{min} \le a \le b \le \mbox{max}$）．

$$ \Pr(a \le X \le b) = \int_a^b f(x) dx = \int_a^b \frac{1}{\mbox{max} - \mbox{min}} dx $$

最小値0，最大値1の一様分布の場合は次のとおり（ただし $0 \le a \le b \le 1$）．
...
$$ \texttt{punif}(x, \text{min}, \text{max}) = \int_{\text{min}}^{x} f(u) du = F(x) = \Pr(X \le x) $$
$$ \text{where} \quad f(u) = \frac{1}{\text{max} - \text{min}} $$
...
$$ \mu = E(X) = \sum_j x_j f(x_j) $$

$$ \sigma^2 = V(X) = E[(X-E(X))^2] = \sum_j (x_j - E(X))^2 f(x_j) = E(X^2) - (E(X))^2 $$ 
...
```{r dice_variance}
sum((dice - mean(dice))^2 * (1/6))  # variance: sum[(x-E(X))^2 * f(x)]
```
...
* `for` 文を使って，乱数生成，平均と分散の計算，記録を行う
   * 計算結果を格納するオブジェクトを `runif_mean` と `runif_var` という名前で作成．`NULL` を代入して空のオブジェクトを作成
   * `c(直前までの計算結果のベクトル, 今回の計算結果)` で append （値を末尾に追加）
...
正規分布に従う確率変数 $X_i \sim N(0, 1)$ の実現値の標本平均（累積平均） $\bar{x}_n = (1/n) \sum_{i=1}^n x_i$ がサンプルサイズ $n$ とともにどのように変化するかを観察する．
...
サンプルサイズが十分に大きいとき，平均 $\mu$ 分散 $\sigma^2$ の母集団から得られた標本の平均 $\bar{X}$ は近似的に正規分布に従う．
...
* サンプルサイズ $n$ が大きくなると標本平均は母平均に近い値を取り，標本平均の標本分布は正規分布に近似される（例外あり）．
...
この性質は，たとえばt検定による平均値の差の検定を適用してよいか判断する際の次の条件と関連する．
...
「正規分布 $N(1,2^2)$ から乱数をドローして標本平均を計算し標準化する」を繰り返し，その分布を調べる．

```{r xbar_std_sigma}
z_list <- NULL
for (i in 1:10000) {
  z_i <- (mean(rnorm(n = 5, mean = 1, sd = 2)) - 1) / (2/sqrt(5))
  z_list <- c(z_list, z_i)
}
...
t_list <- NULL
for (i in 1:10000) {
  data_i <- rnorm(n = 5, mean = 1, sd = 2)
  t_i <- (mean(data_i) - 1) / (sd(data_i)/sqrt(5))
  t_list <- c(t_list, t_i)
}
hist(t_list, probability = TRUE, breaks = 200, xlim = c(-6, 6), ylim = c(0, 0.4))
t_vals <- seq(-6, 6, length.out = 100)
lines(t_vals, dt(t_vals, df=5-1), col = "blue", lwd = 2)  # t(n-1) 
lines(t_vals, dnorm(t_vals), col = "red", lty = 2)  # N(0,1) (for reference) 
...
$t$-検定を行う`t.test`という関数は信頼区間もあわせて出力してくれる．
<!--
...
標本の比率 $p$，サンプルサイズ $n$，z 値 (such that $\Pr(Z \le z) = 1 - \alpha/2$) より次のように母比率の信頼区間が計算される．
...
```{r ci_prop}
x <- 148; n <- 382  # intervention B
p <- x / n
p
p - qnorm(p = .975) * sqrt(p * (1 - p) / n)  # lower limit of 95% CI
p + qnorm(p = .975) * sqrt(p * (1 - p) / n)  # upper limit
```
...
- $p$ 値を計算
    - $p$ 値とは，帰無仮説が正しい場合に，検定統計量が実際に観察された値あるいはそれ以上に極端な値を取る確率として定義される
...
もし実際に観察された検定統計量の値がこの分布の端（たとえば $t=2.4$）に位置するのであれば，それは「帰無仮説が正しいときに得られる可能性が高い値（またはその範囲）から乖離した平均値が観察されていること」を意味する．
...
設定上，帰無仮説は真である（すなわち，帰無仮説が棄却されないことが期待される）が，現実には偶然に大きな値（または小さな値）ばかりがドローされることで標本平均が 0 から大きく乖離し，帰無仮説が棄却される場合がありうる．
...
中心極限定理による正規近似が適用できないような標本平均に対して $t$ 検定を適用したらどうなるだろうか？
...
- 独立した2標本の検定
    - 例：日本企業のアルバイトの時給 vs. 米国企業のアルバイトの時給　（いずれも円に換算したもの）
    - Welchのt検定（通常用いられる手法）
...
t.test(wage_jp, wage_us)  # default: Welch's t-test (allowing for unequal variances) 
t.test(wage_jp, wage_us, var.equal = TRUE)  # t-test (assuming equal variances) 
```
...
<!--
一般的には，sex変数のbase categoryによって（差の大きさは変わらず）差の向きが変わる場合がある．
...
例：A と B という2つの母集団の平均がどちらも0に等しいかどうかを検定したい．帰無仮説は「$\mu_A = \mu_B = 0$」である．このとき，この帰無仮説を $\mu_A=0, \mu_B=0$ という2標本の平均の差の検定が適用できる2つの帰無仮説を組み合わせたものとして捉え，「2つの検定を行ってどちらか1つでも帰無仮説が棄却されたら大元の帰無仮説を棄却する」という判定方法がもし有効であれば，検定は簡単である．しかし，この判定方法は有効ではない．
...
このような多群間の比較を行いたいときは，分散分析（ANOVA）が適用できる場合がある．
