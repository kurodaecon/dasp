...
dta ファイルを読み込むために `haven` パッケージの `read_dta` 関数を使う．
（パッケージについては [データのハンドリングと可視化](https://kurodaecon.github.io/dasp/html/organizing_data.html) を参照．）

既に  `tidyverse` パッケージをインストールしている場合は `haven::read_dta` のみ行う．

```{r install_haven_data_read, eval = F}
install.packages("haven")  # install haven package 
cons <- haven::read_dta(file.choose())  # select the .dta file to load in the GUI 
```

既に `haven` パッケージまたは `tidyverse` パッケージをインストール済みであれば，次のようにする．

```{r data_read, message = F, eval = F}
library(haven)
library(tidyverse)
cons <- haven::read_dta(file.choose())
```

Working directory を設定してそこに保存した .dta ファイルを読み込む場合は次のようにする．

```{r setwd_data_read, message = F}
setwd("c://ws_stat")
cons <- haven::read_dta("Miller2016_MTurk.dta")  # load the .dta file from the working directory 
```
...
```{r mean_con_index_by_conlib_base}
cons$con_index_rep2[cons$Conservative == 0] |> mean(na.rm = TRUE)  # リベラル派の「保守派が好む陰謀論」の支持
cons$con_index_rep2[cons$Conservative == 1] |> mean(na.rm = TRUE)  # 保守派の「保守派が好む陰謀論」の支持
cons$con_index_dem4[cons$Conservative == 0] |> mean(na.rm = TRUE)  # リベラル派の「リベラル派が好む陰謀論」の支持
cons$con_index_dem4[cons$Conservative == 1] |> mean(na.rm = TRUE)  # 保守派の「リベラル派が好む陰謀論」の支持
```
