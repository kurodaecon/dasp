...
### Advanced: Properties of fitted values and residuals 
...
```{r reg_check_pred}
alpha_hat <- lm(Fertility ~ Examination, data = swiss)$coef[1]
beta_hat <- lm(Fertility ~ Examination, data = swiss)$coef[2]
...
$$ \mathbf{X} = \left[ \matrix{X_{11} & X_{12} & X_{13} & \cdots \\ X_{21} & X_{22} & X_{23} & \cdots \\ X_{31} & X_{32} & X_{33} & \cdots \\ \vdots & \vdots & \vdots & \ddots} \right], \quad \mathbf{Y} = \left[ \matrix{Y_1 \\ Y_2 \\ Y_3 \\ \vdots} \right] , \quad \mathbf{\beta} = \left[ \matrix{\beta_1 \\ \beta_2 \\ \beta_3 \\ \vdots} \right] $$
...
残差二乗和 $Q$ を $\mathbf{\beta}$ で微分して 0 とおくことでOLS推定量が得られる．

$$ Q = \mathbf{u}'\mathbf{u} = (\mathbf{Y} - \mathbf{X} \mathbf{\beta})' (\mathbf{Y} - \mathbf{X} \mathbf{\beta}), \quad \frac{\partial Q}{\partial \beta} =0 $$
