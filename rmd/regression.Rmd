...
GitHubにアップロードしている次の3つの画像ファイルも
.Rmd ファイルと同じディレクトリに保存する必要があります
  regression_confounder.png
  regression_mediator.png
  regression_collider.png
-->
...
このページで使用するデータは，カーセンサーnetという中古車情報サイトに掲載されている [人気車種ランキング](https://www.carsensor.net/ranking/pop/) Top 80 の車種に関する車種レベルの属性．
...
`weight` の回帰係数は「`weight` が1単位大きい車種では `price` は 0.37 単位高い傾向にある」＝「車両重量が 1kg 重い車種の価格は3,700円高い」と解釈される．
...
- `weight` = 1,000kg の平均価格（回帰直線上の予測価格）は $-170 + 0.37 \cdot 1000 = 200$ 万円
...
最小二乗法とは，誤差 $u = Y - (\alpha + \beta X_i)$ （推定では観察可能な残差 $\hat{u}_i = Y_i - \hat{Y}_i = Y_i - (\hat{\alpha}+\hat{\beta}X_i)$ を用いる）の二乗和を最小化するように係数パラメタを求める方法である．
...
optim(par = 0, fn = obj_function, method = "BFGS")  # 準ニュートン法を利用
```
...
sum_of_squared_residuals(coef_vector = c(0, 1))  # alpha=0, beta=1
sum_of_squared_residuals(coef_vector = c(0, 0.5))  # alpha=0, beta=0.5
...
res <- y - (alpha_hat + beta_hat * x)  # redefine: residuals 
...
# qt(p = 0.975, df = 1000 - 2)  # 1.962344
hist(t_list, breaks = 100)
abline(v = qt(p = c(0.025, 0.975), df = 1000 - 2), col = "red")
...
- 重回帰分析によって因果効果を推定するためには，処置変数の条件付き独立性（かなり大雑把に言うと，交絡変数がすべてコントロール変数として正確な関数形でモデルに含められていること）が成立していなければならず，この仮定を社会経済データを用いた実証分析において満たすことは容易ではない．
...
詳しくは [VanderWeele (Annual Review of Public Health 2016)](https://doi.org/10.1146/annurev-publhealth-032315-021402) を参照．
...
- $W$ をコントロールしない場合：直接効果 $\beta_{X \to Y} = 2$ と間接効果 $\beta_{X \to W} \times \beta_{W \to Y} = 5 \times 10 = 50$ を合わせたものが推定される
...
「$X \to W \to Y$」のような媒介変数 $W$ について「$W \leftarrow U \to Y$」とする変数 $U$ があったとする．
この変数 $U$ は観測できない要因（unobservables）としよう．

例：

```{r example_for_collider_with_mediator, echo = FALSE, out.width = "60%"}
knitr::include_graphics("regression_collider.png")
```

```{r collider_with_mediator}
set.seed(0)
x <- rnorm(n = 100)
u <- rnorm(n = 100)
w <- 5 * x + 30 * u + rnorm(n = 100)
y <- 2 * x + 10 * w + 50 * u + rnorm(n = 100)
# pairs(data.frame(x, y, w, u))
summary(lm(y ~ x))$coef
summary(lm(y ~ x + w))$coef
```
-->
...
上の結果から「$|0.24| < |0.78|$ なので `ps` の方が `price` の変動をより説明する」と結論付けることはできない．
...
「$|90.8| > |58.7|$ なので `weight` の方が `price` の変動とより強く関連している」と言える．
...
c(mean(car$price_s), mean(car$weight_s), mean(car$ps_s, na.rm = TRUE))  # should be 0 
c(sd(car$price_s), sd(car$weight_s), sd(car$ps_s, na.rm = TRUE))  # should be 1 
lm(price_s ~ weight_s + ps_s, car)$coef
```

「$|0.59| > |0.38|$ なので，相対的には `weight` の方が `price` の変動をよりよく説明している」くらいのことは言える．

ただし，以上の説明は厳密でも明瞭でもないので，レポート・論文での利用は推奨できない．
...
この回帰係数 77 は「ハイブリッド車はハイブリッド車以外と比較して平均的に 77 万円高い」ことを意味している（ただし統計的に有意ではない）．
...
## Quadratic and interaction terms / 二次・交差項
...
よって，「1 kg重い自動車の価格は約0.13%高い」と解釈できる．
...
$$ \Delta \% := 100 \times \frac{f(x+1) - f(x)}{f(x)} = 100 \times \frac{\exp(\alpha + \beta (x+1)) - f(x)}{f(x)} $$
$$ = 100 \times \frac{f(x) \cdot \exp(\beta) - f(x)}{f(x)} = 100 \times [ \exp(\beta) - 1 ] $$
...
両辺に対数を取る場合は「$x$ が 1% 大きな値を取る時に $y$ が $\beta$ % 高い値を取る」と解釈される．
...
いずれか一つのカテゴリーをベースラインとする（定数項に加えてすべてのカテゴリーをダミー変数として含めると多重共線性により推定できなくなるため，一つは除外しなければならない）．
...
```
<!--
knitr::kable(head(car[, c("maker", "Honda", "Mazda", "Nissan", "Subaru", "Suzuki", "Toyota")]))
-->
...
$X^c = X - \bar{X}$ とするとき，モデルの決定係数と回帰直線そのものは変わらないが，一部の係数の推定値と統計的有意性は変わる．

cf. 前掲「Standardized partial regression coefficient / 標準偏回帰係数」

例１：

$$ \mbox{Price} = \alpha + \beta \ \mbox{Width} + u, $$
$$ \mbox{Price} = \alpha^c + \beta^c \ \mbox{Width}^c + u = (\alpha^c - \beta^c \ \bar{\mbox{Width}}) + \beta^c \ \mbox{Width} + u $$

- $\hat{\alpha}$ は Width=0 における Price の平均であるが，$\hat{\alpha}^c$ は Width=mean(Width) における Price の平均を表す．係数はもちろん標準誤差も異なるので，統計的有意性も異なる．
- $\hat{\beta}$ と $\hat{\beta}^c$ はどちらも Width が1単位大きい値を取る時に Price がどれだけ大きい／小さい値を取るかを表す．係数の推定値も標準誤差も同じなので，統計的有意性も当然同じ．

```{r centering_base}
lm1 <- lm(price ~ width, car)
lm2 <- lm(price ~ I(width - mean(width)), car)
huxtable::huxreg(lm1, lm2)
lm2$coef[1] - lm2$coef[2] * mean(car$width)  # should be identical to alpha in model (1) 
```

例２：

$$ \mbox{Price} = \alpha + \beta_1 \ \mbox{Width} + \beta_2 \ \mbox{Weight} + \beta_3 \ \mbox{Width} \times \mbox{Weight} + u \\ = \alpha + \beta_1 \ \mbox{Width} + (\beta_2 + \beta_3 \ \mbox{Width} ) \mbox{Weight} + u, $$
$$ \mbox{Price} = \alpha^c + \beta_1^c \ \mbox{Width}^c + \beta_2^c \ \mbox{Weight} + \beta_3^c \ \mbox{Width}^c \times \mbox{Weight} + u \\ = \alpha^c + \beta_1^c \ \mbox{Width}^c + (\beta_2^c + \beta_3^c \ \mbox{Width}^c) \mbox{Weight} + u $$

- Weight が1単位大きい値を取るときにどれだけ Price が異なる値を取るかが Width によって異なる（Weight の係数に異質性がある）ことを表現するために，交差項を用いる．
- Centering しない場合，$\hat{\beta}_2$ は Width=0 における Weight の係数として解釈される．Width 変数は平均 1.7（最小 1.5，最大 1.9）であり，Width=0 における係数は解釈が難しい．下に示す通りこの係数は負に推定されており（重いほど安い），これは直感に反する．
- Centering する場合，$\hat{\beta}_2^c$ は Width=mean(Width) における Weight の係数として解釈される．直感通り正に推定されている．
- 交差項の係数（・標準誤差）は Centering の影響を受けない．

```{r centering_interaction}
lm2 <- lm(price ~ weight*width, car)
lm3 <- lm(price ~ weight*I(width - mean(width)), car)
huxtable::huxreg(lm2, lm3)
```
...
# car[, c("model", "price", "pred0", "pred1")] |> head(3)
knitr::kable(car[, c("model", "price", "pred0", "pred1")] |> head(3))
predict(lm1, newdata = data.frame(weight = 1000, ps = 100))
```
<!--
DT::datatable(car[, c("model", "price", "pred0", "pred1")])
-->
...
- Step 3. $\widehat{\log y} = \hat{\alpha} + \hat{\beta} x$
    - 当てはめ値は $\widehat{\log{(y)}}$ （対数スケール上の当てはめ値）であって $\log{(\hat{y})}$ （元スケール上の予測値の対数）ではないことに注意．
- Step 4. 
...
$$ \hat{\sigma}^2 = \frac{1}{n - K - 1} \sum_{i = 1}^n \hat{u}_i^2 \quad \mbox{where} \quad \hat{u}_i = \log(Y_i) - \widehat{\log(Y_i)} $$
...
knitr::kable(head(car[, c("model", "price", "pred_correct", "pred_incorrect")], 3))
```
...
2つ目のモデルの値が最も小さいが，2つ目と3つ目の差は十分に小さい（経験則として2を下回っている）ため，2つ目と3つ目のモデルはほとんど同程度と判断できる．
...
* 最小二乗法とは誤差（残差）の二乗が小さくなるように係数パラメタを求める原理
* XがYに与える影響を推定しようとする場合，どのような変数をコントロールしなければならないか，逆にどのような変数はコントロールしてはならないか，を熟考する
* ダミー変数を用いることで，質的変数（カテゴリー変数として記述される情報）を回帰分析の枠組みに組み込むことができる
