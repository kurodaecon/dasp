...
* RStudioを利用：Rをインストールしてから [RStudioをインストール](https://posit.co/download/rstudio-desktop/)
...
```{r obj_char}
univ_name <- "Waseda"
paste(univ_name, "University")  # "paste" concatenates strings
```
...
```{r vector}
c(1, 2, 6)  # combine values into a vector
```
...
x[x > 1]
x > 1  # logical index (logical vector) 
...
```{r vectors}
sum(c(1, 2, 6) - 4:6)  # sum of (1-4), (2-5), and (6-6) = sum of -3, -3, and 0 
x <- c(1, 2, 6)
x2 <- c(1, 3, 4)
x + x2
```

同じ長さの数値ベクトルの内積（スカラー積）は `%*%` で計算される（$\mathbf{x}'$ は $\mathbf{x}$ の転置）．
...
* `sex`: female or male 
* `age`: 年齢
* `sibsp`： 同船するきょうだい（siblings）と配偶者（spouses）の数
...
注：R Markdownの実行環境や各種設定にも依存すると思われるが，.rmd ファイルを開いてチャンクごとに実行する場合，チャンクの実行が終わると作業ディレクトリがリセットされる．
...
「割り切れる」とは「割り算のあまり（余り）が0」ということ．割り算のあまりは `割られる数 %% 割る数` で求められる．
...
また，ある条件を満たすかどうかによって異なる処理を行うには `if, else` を使う．
...
```
<!-- 上記のように single logical value を返す場合は && を使うことができる． -->
...
$$ \left[ \left[ \begin{matrix}1 \\ 2 \\ 3 \end{matrix} \right] , \left[ \begin{matrix}5 \\ 6 \\ 7 \end{matrix} \right] \right] = \left[ \begin{matrix}1 & 5 \\ 2 & 6 \\ 3 & 7 \end{matrix} \right] $$
...
$$ \left[ \begin{matrix} 1 & 5 \\ 2 & 6 \\ 3 & 7 \end{matrix} \right]' = \left[ \begin{matrix} 1 & 2 & 3 \\ 5 & 6 & 7 \end{matrix} \right] $$
...
$$ \left[ \begin{matrix} 1 & 5 \\ 2 & 6 \\ 3 & 7 \end{matrix} \right] \left[ \begin{matrix} 0.1 & 0.2 & 0.3 \\ 0.5 & 0.6 & 0.7 \end{matrix} \right] \\ = \left[ \begin{matrix} 1 \times 0.1 + 5 \times 0.5 & 1 \times 0.2 + 5 \times 0.6 & 1 \times 0.3 + 5 \times 0.7 \\ 2 \times 0.1 + 6 \times 0.5 & 2 \times 0.2 + 6 \times 0.6 & 2 \times 0.3 + 6 \times 0.7 \\ 3 \times 0.1 + 7 \times 0.5 & 3 \times 0.2 + 7 \times 0.6 & 3 \times 0.3 + 7 \times 0.7 \end{matrix} \right] = \left[ \begin{matrix} 2.6 & 3.2 & 3.8 \\ 3.2 & 4 & 4.8 \\ 3.8 & 4.8 & 5.8 \end{matrix} \right] $$
...
$$ \mathbf{A} \mathbf{A}^{-1} = \mathbf{A}^{-1} \mathbf{A} = \mathbf{E} = \left[ \begin{matrix} 1 & 0 & 0 & \cdots \\ 0 & 1 & 0 & \cdots \\ 0 & 0 & 1 & \cdots \\ \vdots & \vdots & \vdots & \ddots \end{matrix} \right] $$

* スカラー（$A \neq 0$）であれば $A A^{-1} = A^{-1} A = 1, A^{-1} = 1/A$ すなわち逆数．逆行列は，これの行列バージョンのようなもの．
* $\mathbf{E}$ は単位行列（対角成分だけが1でそれ以外のすべての要素が0）

2×2 の行列については次のように計算できる．

$$ \left[ \begin{matrix} 1 & 3 \\ 2 & 4 \end{matrix} \right]^{-1} = \frac{1}{1 \times 4 - 3 \times 2} \left[ \begin{matrix} 4 & -3 \\ -2 & 1 \end{matrix} \right] = \left[ \begin{matrix} -2 & 1.5 \\ 1 & -0.5 \end{matrix} \right] $$
...
$$ \left[ \begin{matrix} 1 & 3 \\ 2 & 4 \end{matrix} \right] \left[ \begin{matrix} -2 & 1.5 \\ 1 & -0.5 \end{matrix} \right] = \left[ \begin{matrix} -2 & 1.5 \\ 1 & -0.5 \end{matrix} \right] \left[ \begin{matrix} 1 & 3 \\ 2 & 4 \end{matrix} \right] = \left[ \begin{matrix} 1 & 0 \\ 0 & 1 \end{matrix} \right] $$
...
$$ \det \left[ \begin{matrix} 1 & 0 \\ 1 & 0 \end{matrix} \right] = 1 \times 0 - 0 \times 1 = 0 $$
