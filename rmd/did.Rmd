...
$\dot{y}$ を demean (over individual) した変数 ($\dot{y}_{it} = y_{it} - \bar{y}_i$) とすると，モデルは以下のようになる．
...
summary(lm(crmrte_demean ~ lprbarr_demean + lprbconv_demean + lpolpc_demean + lpolpc_demean, 
...
$$ \Delta y_{it} = \beta_0 + \beta_1 \Delta x_{it} + \epsilon_{it} $$
...
$\dot{y}$ を demean した変数 ($\dot{y} = y_{it} - \bar{y}_i - \bar{y}_t + \bar{\bar{y}}$) とすると，モデルは以下のようになる．
...
| Variable | Control (`nearinc == 0`) | Treatment (`nearinc == 1`) | Difference: NJ-PA | 
| :---: | :---: | :---: | :---: | 
...
<!--
アウトカムに log をとった変数が既に `lrprice` という変数名で格納されている．
```{r kielmc_reg_log}
lm(formula = lrprice ~ nearinc * y81, data = kielmc)
```
-->
...
$$ y_{it} = \beta_0 + \beta_1 x_{it1} + \sum_{t \in \{1, \ldots, 4,6,\ldots,10 \}} \gamma_t (\mbox{Treat})_i \times 1(\mbox{Period}_t = 1) + \alpha_i + \lambda_t + \epsilon_{it} $$
