...
`female` と `male` それぞれで観測数（乗客数）を数える場合．

```{r ft_manual}
head(titanic$sex == "female")  # logical ... TRUE or FALSE
sum(titanic$sex == "female")  # the number of TRUE = the number of female passengers 
sum(titanic$sex == "male")  # the number of male passengers 
```

### Count missing values / 欠損値の数をカウントする
...
度数ではなく密度として表示する場合は `freq = FALSE` という引数を追加する．
<!-- 厳密には相対度数ではないとのこと． -->
...
* 「[0, 20)」は 0 以上 20 未満．右端（20）を含まないので `right = FALSE` 引数を追加
...
```{r var}
x <- c(1, 2, 6)
x - 3  # deviation from the mean; mean(x) = 3 
x - mean(x)
(x - mean(x))^2
```
...
* `rnorm(n = サンプルサイズ, mean = 平均, sd = 標準偏差)` 関数で正規分布と呼ばれる確率分布に従う乱数を生成
   * 母集団の標準偏差を `sd = 1` と設定し，乱数に基づいて計算された分散の平均値が 1 に近いかどうかを確認する
...
なお，$V(\bar{X})$ は $n \to \infty$ で 0 になることから，サンプルサイズが十分に大きければ標本平均は真の平均に近づく（より正確に言えば確率収束する．「大数の法則」としても知られる．）ことも復習できる．
...
```{r box_age}
boxplot(titanic$age) 
text(1.35, quantile(titanic$age, .75, na.rm = TRUE) + 1.5 * IQR(titanic$age, na.rm = TRUE), labels = "“max” (= Q3+1.5*IQR)")
...
注：箱の下の髭は実際の最小値まで伸ばす．実際の最小値 (age = 0.17) が下の計算式で与えられる “min” より大きいため．<!-- cf. lower/upper fence -->

```{r box_min}
quantile(titanic$age, .25, na.rm = TRUE) - 1.5 * IQR(titanic$age, na.rm = TRUE)
```
...
<!--
Same data but different scale (on y-axis). 
-->
...
`col = rgb(red, green, blue, alpha)` のように引数を追加すれば，RGB形式で色を指定したうえに，透過度を alpha 値（0以上1以下）として指定できる．
...
3つ以上の連続変数について，2変数ごとの散布図を行列形式で表示する．
...
このとき，偏差の積は，平均を基準にした第1象限・第3象限では正，第2象限（・第4象限）では負の値を取る．
...
contour(xx, yy, zz, xlab = "x", ylab = "y", 
        main = expression("Contour of " (x-bar(x))(y-bar(y))), labcex = 1.5)
abline(v = mean(x), h = mean(y), col = "red")
...
$$ \operatorname{Cor}(x, y) = \frac{Cov(x, y)}{\sqrt{s^2_x} \cdot \sqrt{s^2_y}} $$
...
大雑把に言えば，相関係数の大きさはデータに最もよく当てはまる直線（下図では赤色の実線）にデータがどれだけ近いかによって決まる．
...
par(mfrow = c(1, 2))
plot(x2, y2a, ylim = c(-3, 3), main = paste("r =", round(cor(x2, y2a), 2)), ylab = "x2 + small noise")
abline(lm(y2a ~ x2), col = "red")  # Add fitted (regression) line 
plot(x2, y2b, ylim = c(-3, 3), main = paste("r =", round(cor(x2, y2b), 2)), ylab = "x2 + large noise")
...
par(mfrow = c(1, 2))
plot(x3, y3a, ylim = c(-2, 2), main = paste("r =", round(cor(x3, y3a), 2)), ylab = "x3 + noise")
abline(lm(y3a ~ x3), col = "red")
plot(x3, y3b, ylim = c(-2, 2), main = paste("r =", round(cor(x3, y3b), 2)), ylab = "(x3 + noise) / 2")
...
相関係数が 1 や -1 になるのは，一方の変数がもう一方の変数の定数倍・定数加算によって得られるような場合（$\{ (X, Y) \mid Y = a + b X, a,b \in \mathbb{R}, b \neq 0 \}$）．
<!--
...
cor(x, x4b)
par(mfrow = c(1, 2))
plot(x, x4a, cex = 2, ylab = "2x + 3"); abline(lm(x4a ~ x), col = "red")
plot(x, x4b, cex = 2, ylab = "-0.5x + 3"); abline(lm(x4b ~ x), col = "red")
```
